...
class BaseDecodeHead_clips2(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        print("----------need_segdeformer",need_segdeformer)
        print("----------------aux_loss_decode",aux_loss_decode)
        # 配置多个损失
    ...
    def losses(self, seg_logit, seg_label):
        ...
        # print(seg_logit.shape, seg_label.shape)
        k = 4 #num of ratios
        if self.cross_method == 'CAT' or self.cross_method == 'Focal_CAT':
            total = 7
            # print("loss:",seg_logit.shape, seg_label.shape)
            if self.hypercorre and self.cityscape:
                print("here1") #对于backbone只比较了target
                ...
                seg_label_ori=seg_label[:,-1] # target [b,c,h,w]
                # print(seg_label[:,-1].shape)
                seg_label_lastframe=seg_label[:,-1:].expand(batch_size,k,1,h,w).reshape(batch_size*(k),1,h,w)

                if k!=total:
                    seg_label_lastframe_aux=seg_label[:,-1:].expand(batch_size,total-k,1,h,w).reshape(batch_size*(total-k),1,h,w)
                    seg_logit_lastframe_aux=seg_logit[:,num_clips+k:].reshape(batch_size*(total-k),-1,ph,pw) #[b*(total-k),c,h,w]

                                                                                                    
            elif self.hypercorre:
                # print("here2 with CAT")
                if self.self_ensemble2 and seg_logit.shape[1]==seg_label.shape[1]+total:
                    ...
                    seg_label_ori=seg_label.reshape(batch_size*(num_clips),1,h,w)
                    seg_label_lastframe=seg_label[:,-1:].expand(batch_size,k,1,h,w).reshape(batch_size*(k),1,h,w)
                    # print("loss shape",seg_logit.shape,seg_logit[:,num_clips+k:].shape)
                    if k!=total:
                        seg_label_lastframe_aux=seg_label[:,-1:].expand(batch_size,total-k,1,h,w).reshape(batch_size*(total-k),1,h,w)
                        seg_logit_lastframe_aux=seg_logit[:,num_clips+k:].reshape(batch_size*(total-k),-1,ph,pw) #[b*(total-k),c,h,w]
            ...
            if k!=total:
                seg_logit_lastframe_aux = resize(
                    input=seg_logit_lastframe_aux,
                    size=seg_label.shape[3:],
                    mode='bilinear',
                    align_corners=self.align_corners)
            ...
            seg_label_lastframe = seg_label_lastframe.squeeze(1)
            if k!=total:
                seg_label_lastframe_aux = seg_label_lastframe_aux.squeeze(1)
            ...
            for loss_decode in losses_decode:
                if loss_decode.loss_name not in loss:
                    ...
                    loss[loss_decode.loss_name]+= 0.5*loss_decode(
                                    ...
                                    ignore_index=self.ignore_index)
                    if k!=total:
                        loss[loss_decode.loss_name]+= 0.2*loss_decode(
                                    seg_logit_lastframe_aux,
                                    seg_label_lastframe_aux,
                                    weight=seg_weight,
                                    ignore_index=self.ignore_index)
            ...
            if self.hypercorre and self.cityscape:
                ...
                # print("here2 with Focal")
                # print("seg_logit.shape: ",seg_logit.shape,seg_label.shape)
                if self.self_ensemble2 and seg_logit.shape[1]==seg_label.shape[1]+total:
                    # print("here2")
                    assert seg_logit.shape[1]==seg_label.shape[1] + total
                    num_clips=seg_label.shape[1]
                    ...
                    assert chan == 1
                    seg_label_ori = seg_label.reshape(batch_size*(num_clips),1,h,w)
                    seg_label_lastframe = seg_label.reshape(batch_size*(num_clips),1,h,w)
                    # print("loss shape",seg_logit.shape,seg_logit[:,num_clips+k:].shape)
                    if k!=total:
                        assert num_clips == total - k 
                        seg_label_lastframe_aux=seg_label.reshape(batch_size*(num_clips),1,h,w)
                        seg_logit_lastframe_aux=seg_logit[:,num_clips+k:].reshape(batch_size*(total-k),-1,ph,pw) #[b*(total-k),c,h,w]
        ...
        #     ignore_index=self.ignore_index)
        ...
        elif self.cross_method == 'Cluster':
            total = 2
            if not self.need_segdeformer:
                total = 1
            # print("loss:",seg_logit.shape, seg_label.shape)
            if self.hypercorre and self.cityscape: #backbone上只比较了target
                ...
                if not self.aux_loss_decode: 
                    ...
                    total = 5
                    k = 2
                    # print("ok?",self.self_ensemble2,seg_logit.shape[1],seg_label.shape[1]+total)
                    if self.self_ensemble2 and seg_logit.shape[1]==seg_label.shape[1]+total:
                        assert seg_logit.shape[1]==seg_label.shape[1] + total 
                        num_clips=seg_label.shape[1]
                        # assert k == num_clips
                        ...
                        # 这里是一致的
                        # seg_logit_ori = torch.cat([seg_logit_ori,seg_aux_frame],dim=0)
                        # seg_label_ori = torch.cat([seg_label_ori,seg_label_auxframe],dim=0)
                # else:
                #     assert False, "parameters not correct"            
            ...
            for loss_decode in losses_decode:
                ...
                if "loss_backbone" not in loss:
                    ...
                    # 有0.5的权重out和0.2的权重cls_mid 
                    # loss["loss_out"] = 0.5*loss_decode(    
                    #                 seg_logit_lastframe[:batch_size,...],
                    #                 seg_label_lastframe[:batch_size,...],
                    #                 weight=seg_weight,
                    #                 ignore_index=self.ignore_index)

                    # loss["loss_mid_cls"] = 0.4*loss_decode(
                    #                 seg_logit_lastframe[batch_size:,...],
                    #                 seg_label_lastframe[batch_size:,...],
                    #                 weight=seg_weight,
                    #                 ignore_index=self.ignore_index)

                    loss["loss_decode"] = 0.3*loss_decode(
                                    seg_logit_lastframe,
                                    seg_label_lastframe,
                                    weight=seg_weight,
                                    ignore_index=self.ignore_index)
            ...
            if self.need_cluster_loss:
                for _loss_cluster in loss_cluster:
                    if _loss_cluster.loss_name not in loss:
                        loss[_loss_cluster.loss_name] = 0.2 * _loss_cluster(cluster_centers)
                    else:
                        loss[_loss_cluster.loss_name] += 0.2 * _loss_cluster(cluster_centers)
                        
            if self.aux_loss_decode:
                for loss_decode in losses_decode:
                        if "loss_aux" not in loss:
                            loss["loss_aux"] = 0.2 * loss_decode(
                                                        seg_aux_frame,
                                                        seg_label_auxframe,
                                                        weight=seg_weight,
                                                        ignore_index=self.ignore_index) 
