# method from paca_vit 加辅助损失增强一致性
import copy
import os
...
from .fdsf import FDSF,PagFM,SWFG2,FSFM,DU2
...
class Cluster_layer(nn.Module):
    ...
    def forward(self,x,H,W,z=None,mem=None,t=3,threshold=0.95):
        ...
        '''
        res = x
        # print("in_shape",x.shape,mem.shape)
        if mem is not None:
        ...
        cluster_x_z = rearrange(cluster_x_z,"(b t) c n -> b t c n",t=t)
        ...
        # print("center",center.shape,center_x.shape)
        ...
        # 如果这个效果好,可以直接在这里加聚类损失，这样既能满足均值的特性，又可以满足聚类的特性
        C_in = center.mean(dim=1)
        ...
        res = self.norm1(res)    
        src = rearrange(res,"(b t) n c -> (t n) b c",b=C_in.shape[0])
        ...
        out = rearrange(out,"(t n) b c -> (b t) n c",t=t)
...
class hypercorre_topk2(nn.Module):
    ...
    def __init__(self,dim=[64, 128, 320, 512], num_layers=1, t=3, time_decoder_layer=3,embedding_dim=256,num_classes = 124,
                 cross_method='CAT',ratio_fusio = False,num_clusters=150,cluster_with_t=False,aux_loss_decode=False,updata_mem = False):
        super().__init__()
        self.dim=dim
        ...
        self.t = 4 if aux_loss_decode else 1 
    ...
    def forward(self, query_frame, supp_frame,t = None,img_metas=None):
        ...
        for idx in range(self.num_layers):
            if idx == 0:
                x,z,center = self.cluster_blocks[idx](src, H=H, W=W, mem = memory)
            elif idx == 1:
                x,z,center = self.cluster_blocks[idx](x, H=H, W=W, z=z, mem = memory) #聚类学习[b,n,c]
            else:
                x,_,_ = self.cluster_blocks[idx](x, H=H, W=W) # 自身的加强

            # mem到底需不需要更新
            if self.training and self.aux_loss_decode and idx <= 1: # 需要pre做辅助损失
                x = rearrange(x,'(b t) n c -> b t n c', t=T_tg+T_pre)
                mem = x[:,:-1]
                x = x[:,-1]
                if self.updata_mem: #聚类过程需要更新mem
                    memory = rearrange(mem,"b t n c -> (b t) n c")
        ...
        if self.training and self.aux_loss_decode: # 需要pre做辅助损失
            mem = rearrange(mem,"b t (h w) c -> b t c h w",h=H,w=W)
            x = torch.cat([mem,x],dim=1)
        print("out_cluster",x.shape)
        # 同Focal,只有tg得到更新
        # 可学习的原型 (B,num_clips,C,H,W)
        out_cls_mid, cls_tokens =  self.class_token(x)
        out_new = self.trans(x, cls_tokens, out_cls_mid) #bxtxcxhxw
        print("out_trans",out_new.shape)
        ...
        if self.training and self.aux_loss_decode:
            ...
            out_cls_mid = out_cls_mid.squeeze(1)

        out_new=(torch.chunk(out_new, t_out, dim=1))
        out_new=[ii.squeeze(1) for ii in out_new] # focal情况下就是对所有帧的分割细化(还对目标帧进行了更新)，CAT就是对目标帧的分割细化
        return out_new,out_cls_mid,center #[b,c,h,w]
