...
def StaticDynamicAttention(query,key,value, lv=[1,1], low_res_weights=None, H1=None,W1=None):
    ...
    for j in range(h_ration): # 切块
        for i in range(w_ration):
          ...
          # 直接把t展开算的
            query_block = query[:, start_h:end_h, start_w:end_w, :].permute(0, 3, 1, 2) # [B,C,h,w]
            key_block = key[:,  :, start_h:end_h, start_w:end_w,:].permute(0, 4, 1, 2, 3) # [B,C,t,h,w]
            value_block = value[:, :, start_h:end_h, start_w:end_w, :].permute(0, 4, 1, 2, 3) # [B,C,t,h,w]
            # 计算相似度
...
class hypercorre_topk2(nn.Module):
    ...
    def forward(self, query_frame, supp_frame,v_frame,lv = [[1,1],[2,2],[4,4],[8,8]],debug = False):
        ...
        for ii in range(0,len(supp_frame)-1): #1/32 - 1/8 切块lv[ii]
            ...
            query = supp_qkv_all[ii].squeeze(1)  #[b,h,w,c]
            key = query_qkv_all[ii]  #[b,num_ref,h,w,c]
            value = v_frame[ii].permute(0,1,3,4,2)      #[b,num_ref,h,w,c] 
