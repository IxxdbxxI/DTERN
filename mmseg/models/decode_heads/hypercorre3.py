...
def get_sim_mask(query,key,threshold=0.9):
    '''
        input:
            query: [B, C, h, w]
            key: [B, C,t, h, w]
...
def StaticDynamicAttention(query,key,value, lv=[1,1], low_res_weights=None, H1=None,W1=None):
    ...
    out = torch.zeros(B,C,H,W).to(query.device)
    none_update_query = []
    none_update_value = []
    none_update_key = []
    none_point_indices = []
    ...
    for j in range(h_ration): # 切块
        for i in range(w_ration):
          ...
          # 直接把t展开算的
            query_block = query[:, :, start_h:end_h, start_w:end_w] # [B,C,h,w]
            key_block = key[:,  :,:, start_h:end_h, start_w:end_w].permute(0, 2, 1, 3 , 4) # [B,C,t,h,w]
            value_block = value[:, :, :, start_h:end_h, start_w:end_w].permute(0, 2, 1, 3 , 4) # [B,C,t,h,w]
            # 计算相似度
            sim,mask_qk,mask_kq = get_sim_mask(query_block,key_block) # sim:[B,h*w,t*h*w], mask_kq:[B,h,w],mask_qk:[B,t,h,w],qk是哪些k对整个q有用更新k,v,kq更新q
            
            # 用更新的k，v计算这个区域
            # 这里可以有两种：
            # 1. 利用k,v的mask更新k,v 
            attn_block = sim.softmax(dim=-1)
            # print("attn",attn_block.shape)
            out_block = attn_block @ value_block.flatten(2).transpose(1, 2) # [B,h*w,thw] * [B,thw,C] = [B,h*w,C]
            # print("out_block",out_block.shape)  
            # print("end-start",end_h-start_h,end_w-start_w)
            out_block = out_block.transpose(1, 2).view(B,C,end_h-start_h,end_w-start_w)
            ...
            ratios_out.append(mask_kq.sum()/((end_h-start_h)*(end_w-start_w)))
            none_mask_kq = ~mask_kq #[b,H,W]
            # print("mask",none_mask_kq.shape)
            # 使用掩码选择数据
            query_block_data = [query_block[b, :, none_mask_kq[b]] for b in range(B)]#[b (C,S')]
            ...
            key_block_data = [key_block[b, :, none_mask_qk[b]] for b in range(B)]
            # print("key_block_data",key_block_data[0].shape,len(key_block_data))
            none_update_key.append(key_block_data) # (b,(C,S1))
            value_block_data = [value_block[b, :, none_mask_qk[b]] for b in range(B)]
            none_update_value.append(value_block_data) #[B,C,S1]
    ...
    for ii in range(B):
        ...
        for query_block_data,key_block_data,value_block_data,mask_data in zip(none_update_query,none_update_key,none_update_value,none_point_indices):
            query_out.append(query_block_data[ii]) #[C,S] 
            key_out.append(key_block_data[ii]) #[C,S1]
            value_out.append(value_block_data[ii]) #[C,S1]
            h,w = mask_data[ii].shape
            mask_out[x:x+h,y:y+w] = mask_data[ii] #[H,W]
            y = (y + w) % W
        ...
        out[ii].masked_scatter_(expanded_mask, _out)
...
class Conv3_1_block(nn.Module): # 既然linear和point_wise类似，那么采用可分离卷积处理
    def __init__(self, in_channels, out_channels,bias=True):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels, 3, stride=1,bias=bias,padding=1)
...
# all-t , cnn_qk, linear_v 一起算的时间，直接对时间进行的融合，所有时间被视作等价的，但是在计算相似度中，所有token被视为平等
class hypercorre_topk2(nn.Module):
    ...
    def forward(self, query_frame, supp_frame,v_frame,lv = [[1,1],[2,2],[4,4],[8,8]],debug = False):
        """ Forward function.
        ...
        for ii in range(0,len(supp_frame)-1): #1/32 - 1/8 切块lv[ii]
            ...
            query = supp_qkv_all[ii].squeeze(1)  #[b,c,h,w]
            key = query_qkv_all[ii]  #[b,num_ref,c,h,w]
            value = v_frame[ii]     #[b,num_ref,c,h,w] 
            # print("shape",query.shape,key.shape,value.shape)
            dy_out = StaticDynamicAttention(query=query,key=key,value=value,lv=lv[ii]) #[b,c,h,w]
            out.append(dy_out)
        return out
