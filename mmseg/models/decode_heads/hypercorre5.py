...
class hypercorre_topk2(nn.Module):
    ...
    def __init__(self,dim=[64, 128, 320, 512], num_layers=1, t=3, time_decoder_layer=3,embedding_dim=256):
        ...
        for idx in range(3):
            ...
            self.tmp_blocks.append(TimeLayer(
                hidden_channel=dim[idx],
                feedforward_channel=dim[idx]*4,
                num_head=num_heads[idx],
                decoder_layer=time_decoder_layer
            ))
        
    def forward(self, query_frame, supp_frame,img_metas=None):
        """ Forward function.
        ...
        for ii in range(0,len(supp_frame)-1): #1/32 - 1/8 切块lv[ii]
            memory = query_frame[ii]
            src = supp_frame[ii]
            B,num_clips,C,H,W = memory.shape
            for _ in range(self.num_layers):
                self.pre_isa_blocks[ii][_].H,self.pre_isa_blocks[ii][_].W = H , W
                self.cpa_blocks[ii][_].H,self.cpa_blocks[ii][_].W = H , W
                self.post_isa_blocks[ii][_].H,self.post_isa_blocks[ii][_].W = H , W
            ...
            for _ in range(self.num_layers):
                output = self.pre_isa_blocks[ii][_](output,memory)
                # print("pre_isa",output.shape)
                output = self.cpa_blocks[ii][_](output) 
                # output = self.cpa_blocks[ii][_](output,memory) # 按理说应该是两个
                # print("cap",output.shape)
                output = self.post_isa_blocks[ii][_](output) #[B*T,N,C] 
                # print("post_isa",output.shape)
            # print("intmp:",output.shape,memory.shape,num_clips)
            output = self.tmp_blocks[ii](output,memory,num_clips) #[B*T,N,C]
            # print("tm_out",output.shape)
            output = output.view(B,num_clips,H*W,C).permute(0,2,1,3).flatten(2,3) #[B,N,T*C]
            # print("in_tmp_out",output.shape)

            output = self.conv_t_out[ii](output,H,W) #[B,N,C]
            out.append(output.view(B,H,W,-1).permute(0,3,1,2)) #[B,C,H,W]
        return out
