...
class TransformerClassToken3(nn.Module):
    ...
    def forward(self, x, cls_tokens, out_cls_mid):
        ...
        out_cls_mid = self.linear_cls(out_cls_mid) #b,t,n,num_classes -> b,t,n,num_prototypes,这里的映射好像不太合适，
...
class hypercorre_topk2(nn.Module):
    ...
    def __init__(self,dim=[64, 128, 320, 512], num_layers=1, t=3, time_decoder_layer=3,embedding_dim=256,num_classes = 124,cross_method='CAT',ratio_fusio = False):
        super().__init__()
        self.dim=dim
        ...
        assert cross_method in ['CAT', 'Focal','Focal_CAT']
    ...
    def forward(self, query_frame, supp_frame,t = None,img_metas=None):
        ...
        if self.cross_method == 'CAT':
        ...
        elif self.cross_method == 'Focal_CAT':
            # Focal
            _c_further = torch.cat([memory,src],dim=1) # [b,all_num_clips,c,h,w]
            output = self.decoder_focal(_c_further) #这里前几帧都没更新，只有最后一帧更新 # [b,all_num_clips,c,h,w]
            ...
            src = src.permute(0,1,3,4,2).flatten(0,1)
            output = src.view(-1,H*W,C)
            memory = memory.permute(0,1,3,4,2).flatten(0,1)
            memory = memory.view(-1,H*W,C)
            # print("input",output.shape,memory.shape)
            for _ in range(self.num_layers//2):
                output = self.pre_isa_blocks[0][_](output,memory)
            ...
            # output = self.tmp_blocks[0](output,memory,num_clips) #[B*T,N,C]
            # print("tm_out",output.shape)
            output = output.view(B,num_clips,H*W,C).permute(0,1,3,2).view(B,num_clips,C,H,W)
