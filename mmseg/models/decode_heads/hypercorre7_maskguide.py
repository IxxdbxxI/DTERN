...
class AgentAttention(nn.Module):
    ...
    def __init__(self, dim, patch_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 shift_size=0, num_classes=124,t=3, **kwargs): # agent_num = num_classes
        ...
        self.patch_size = patch_size  # Wh, Ww
        ...
        self.an_bias = nn.Parameter(torch.zeros(num_heads, num_classes, patch_size[0], patch_size[1])) #[h,cls,h,w]
        self.na_bias = nn.Parameter(torch.zeros(num_heads, num_classes, patch_size[0], patch_size[1])) #[h,cls,h,w]

        self.ah_bias = nn.Parameter(torch.zeros(1, num_heads, num_classes, patch_size[0], 1)) #[1,h,cls,h,1]
        self.aw_bias = nn.Parameter(torch.zeros(1, num_heads, num_classes, 1, patch_size[1])) #[1,h,cls,1,w]

        self.ha_bias = nn.Parameter(torch.zeros(1, num_heads, patch_size[0], 1, num_classes)) #[1,head,h,1,cls]
        self.wa_bias = nn.Parameter(torch.zeros(1, num_heads, 1, patch_size[1], num_classes)) #[1,head,1,w,cls]
    ...
    def forward(self, x, mem=None,cls_toekn_x=None,cls_token_mem=None,mask=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C)
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
            cls_toekn_x:(num_windows*B,N,cls)
            mem: [num_windows*B*t, N, C]
            cls_token_mem: [num_windows*B, t, N, cls]
        """
        b, n, c = x.shape
        # print("x",x.shape,mem.shape)
        # _b,n,c = mem.shape
        mem = mem.view(b,-1,n,c)
        _b = mem.shape[0] * mem.shape[1]
        t = mem.shape[1]
        ...
        kv = self.kv(mem).reshape(_b,n, 2, c).permute(2, 0, 1, 3)
        # print("kv",kv.shape)
        k, v =  kv[0], kv[1]  # make torchscript happy (cannot use tensor as tuple)
        # q: b, n, c  k, v: bt, n, c


        cls_toekn_x = cls_toekn_x.transpose(-1,-2) @ x # b,cls,N * b,N,C -> b,cls,C
        cls_token_mem = cls_token_mem.transpose(-1,-2) @ mem # b,t,cls,N * b,t,N,C -> b,t,cls,C
        # print("q,k,v",q.shape,k.shape,v.shape)
        q = q.reshape(b, n, num_heads, head_dim).permute(0, 2, 1, 3) 
        k = k.reshape(b,t, n, num_heads, head_dim).permute(0, 1, 3, 2, 4) # b,t,n,hc -> b,t,h,n,hc
        v = v.reshape(b,t, n, num_heads, head_dim).permute(0, 1, 3, 2, 4)

        cls_toekn_x = cls_toekn_x.reshape(b, self.num_classes, num_heads, head_dim).permute(0, 2, 1, 3) #b,h,cls,hc
        cls_token_mem = cls_token_mem.reshape(b, t, self.num_classes, num_heads, head_dim).permute(0, 1, 3, 2, 4) #b,t,h,cls,hc

        # print("pos",self.an_bias.shape) # [h,cls,h,w]
        position_bias1 = nn.functional.interpolate(self.an_bias, size=self.patch_size, mode='bilinear') #[h,cls,h,w]
        position_bias1 = position_bias1.reshape(1,1, num_heads, self.num_classes, -1).repeat(b,t, 1, 1, 1) #[b,t,h,cls,h*w]
        position_bias2 = (self.ah_bias + self.aw_bias).reshape(1,1, num_heads, self.num_classes, -1).repeat(b,t, 1, 1, 1) #[1,h,cls,h,w] + [1,h,cls,1,w] -> [b,t,h,cls,h*w]
        position_bias = position_bias1 + position_bias2
         
        # 1.自己和自己交互  2.自己和别人交互
        agent_attn = self.softmax((cls_token_mem * self.scale) @ k.transpose(-2, -1) + position_bias) # b,t,h,cls,hc * b,t,h,hc,n -> b,t,h,cls,n

        agent_attn = self.attn_drop(agent_attn)
        agent_v = agent_attn @ v # b,t,h,cls,n * b,t,h,n,hc -> b,t,h,cls,hc

        agent_bias1 = nn.functional.interpolate(self.na_bias, size=self.patch_size, mode='bilinear') #[head,cls,h,w]
        agent_bias1 = agent_bias1.reshape(1, num_heads, self.num_classes, -1).permute(0, 1, 3, 2).repeat(b, 1, 1, 1) #[b,head,h*w,cls]
        agent_bias2 = (self.ha_bias + self.wa_bias).reshape(1, num_heads, -1, self.num_classes).repeat(b, 1, 1, 1) #[1,head,h,1,cls] + [1,head,1,w,cls] -> [b,head,h*w,cls]
        agent_bias = agent_bias1 + agent_bias2 #[b,head,h*w,cls]

        q_attn = self.softmax((q * self.scale) @ cls_toekn_x.transpose(-2, -1) + agent_bias) # b,h,n,hc * b,h,hc,cls -> b,h,n,cls
        q_attn = self.attn_drop(q_attn)
        x = q_attn.unsqueeze(1) @ agent_v # b,1,h,n,cls * b,t,h,cls,hc -> b,t,h,n,hc

        x = x.transpose(2, 3).reshape(_b, n, c)
        v = v.transpose(2, 3).reshape(_b, h, w, c).permute(0, 3, 1, 2)
        dw_v = self.dwc(v).permute(0, 2, 3, 1)
        # print("x,dw_v",x.shape,dw_v.shape)
        x = x + dw_v.reshape(_b,n, c)
        return x.view(b,-1,n,c) # (b,t,n,c)
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, patch_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., rpe=True,attn_type="ipsa",select = False,seg_mask_use=None,num_classes=124,t=3):
        super().__init__()
        self.dim = dim
        ...
        self.seg_mask_use = seg_mask_use
        if seg_mask_use == "cluster_Agent":
            self.agent_attention = AgentAttention(dim, patch_size, num_heads, qkv_bias, qk_scale, attn_drop, proj_drop,num_classes=num_classes,t=t)
    ...
    def forward(self, x,memory=None,seg_mask=None):
        """
        ...
        if self.attn_type == "ipsa" and self.seg_mask_use == "cluster_reduce_cmp" and seg_mask is not None: # A -> B
                seg_mask = seg_mask.view(B_, t+1, N, -1) #[b*np,t,pp,c]  
                tg_x = seg_mask[:,-1]
                tg_memory = seg_mask[:,:-1]
                x = tg_x.transpose(-1,-2) @ x  # [b*np,c,pp] @ [b*np,pp,d] -> [b*np,c,d] 
                memory = tg_memory.transpose(-1,-2) @ memory  # [b*np,t,c,pp] @ [b*np,t,pp,d] -> [b*np,t,c,d] A -> B
            
            # elif self.seg_mask_use == "cluster_Agent":
            #     seg_mask = seg_mask.view(B_, -1, N, C) 
            #     tg_x = seg_mask[:,-1] # [b*np,pp,c]
            #     tg_memory = seg_mask[:,:-1] 

        # cluster Agent
        if self.seg_mask_use == "cluster_Agent" and self.attn_type == "ipsa" and seg_mask is not None:
            seg_mask = seg_mask.view(B_, t+1, N, -1) #[b*np,t,pp,c]
            tg_x = seg_mask[:,-1]
            tg_memory = seg_mask[:,:-1]
            x = self.agent_attention(x, memory,cls_toekn_x=tg_x,cls_token_mem=tg_memory) # [b*np,t,N,C]
            
        else:
            q = self.q(x) # [B,  c, C]
            q = q.reshape(B_, x.shape[-2], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3).unsqueeze(1) # nP*B,1, nH, c, C
            ...
            kv=kv.reshape(B_, -1, memory.shape[-2], 2, self.num_heads, C // self.num_heads).permute(3, 0, 1, 4, 2, 5) # nP*B, T, nH, c, C
            k, v = kv[0], kv[1]

            q = q * self.scale
            # print("q,k,v",q.shape,k.shape,v.shape)
            attn = (q @ k.transpose(-2, -1)) #(nP*B, T, nH, c, c)
            ...
            x = (attn @ v).transpose(2, 3).reshape(B_,-1, N, C) # [nP*B, T,nH,  c, c] * [nP*B, T, nH,c, C] -> [nP*B, T,nH, c, C]
        

            if self.attn_type == "ipsa": # B->A
                if self.seg_mask_use == "cluster_reduce_cmp" and seg_mask is not None:
                    x = x.transpose(-1,-2) @ tg_x.unsqueeze(1).repeat(-1,x.shape[1],-1,-1) # [b*np,t,C,c] @ [b*np,c,pp] -> [b*np,t,C,pp] 
                    x = x.transpose(-1,-2)  # [b*np,t,pp,C] 
...
class CATBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, patch_size=7, mlp_ratio=4., qkv_bias=True, 
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_type="ipsa", rpe=True,seg_mask_use = None,num_classes=124,t=3):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        if self.seg_mask_use == "cluster_guide":
            self.cls_fc1 = nn.Linear(dim,1)


        self.attn = Attention(
            dim=dim if attn_type == "ipsa" else self.patch_size ** 2, patch_size=to_2tuple(self.patch_size),
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, rpe=rpe,seg_mask_use=seg_mask_use,num_classes=num_classes,t=t)
    ...
    def forward(self, x,memory = None,seg_mask = None):
        ...
        if self.seg_mask_use is not None and seg_mask is not None:
            seg_mask = seg_mask.flatten(0,1) 
            seg_mask =F.interpolate(seg_mask, size=(Hp,Wp), mode='bilinear', align_corners=False) # [b*t,C,Hp,Wp]
        
        #guide
        if self.seg_mask_use == "cluster_guide" and seg_mask is not None:
            frames = torch.cat([memory.view(B,-1,Hp,Wp,C),x.view(B,-1,Hp,Wp,C)],dim=1) # [B, T, H, W, C]
            frames = frames.flatten(0,1)
            # frames: [B*T, H, W, C]
            frames = frames.flatten(1,2) # [Bt, N, C]
            tg_mask = seg_mask.flatten(2) # [bt,classes,h*w]
            tg_mask = F.softmax(tg_mask,dim=1) 
            gd_x = tg_mask @ frames # [bt,classes,C]
            gd_x = F.sigmoid((gd_x.transpose(-1,-2) @ torch.mean(gd_x,dim=-1,keepdim=True)).squeeze(-1)) # [b,C,1] 拉近类内距离
            frames =  frames + frames * gd_x.unsqueeze(1)
            frames = frames.view(B,-1,Hp,Wp,C)
            memory = frames[:,:-1].reshape(_B,Hp,Wp,C)
            x = frames[:,-1]
            # print("cluster_guide",memory.shape,x.shape)   
        ...
        if self.seg_mask_use in ["cluster_reduce_cmp","cluster_Agent"] and seg_mask is not None:
            # 切分
            seg_mask = seg_mask.permute(0,2,3,1).contiguous() # [b*t,h,w,c] 
            seg_mask = partition(seg_mask,self.patch_size) # [b*t*nP, patch_size, patch_size, C]
            seg_mask = seg_mask.view(B,-1,Hp//self.patch_size*Wp//self.patch_size, self.patch_size, self.patch_size, seg_mask.shape[-1])  # nP*B*T, patch_size*patch_size, C
            seg_mask = seg_mask.permute(0,2,1,3,4,5).contiguous().view(-1, self.patch_size * self.patch_size, seg_mask.shape[-1])  # nP*B*T, patch

        # IPSA or CPSA
        if self.attn_type == "ipsa": #token selection (top-k or k-means)
            # print("self.seg_mask_use",patches.shape,patches_memory.shape)
            # if seg_mask is not None:
            #     print("self.seg_mask_use",seg_mask.shape)

            attn = self.attn(patches,patches_memory,seg_mask)  # [B*nP, T, N, C] 
...
class hypercorre_topk2(nn.Module):
    ...
    def __init__(self,dim=[64, 128, 320, 512], num_layers=1, t=3, time_decoder_layer=3,embedding_dim=256,num_classes = 124,cross_method='CAT',seg_mask_use = None):
        super().__init__()
        self.dim=dim
        ...
        assert cross_method in ['CAT', 'Focal']
        self.cross_method = cross_method
        self.seg_mask_use = seg_mask_use
        # test from cffm cross refiner to replace CAT blks
        if cross_method == 'Focal':
            self.t = t+1
    ...
    def forward(self, query_frame, supp_frame, seg_mask = None, t = None):
        """ Forward function.
        ...
        '''
         seg_mask: mask from backbone [b,num_clips,c,h,w]:表示每个clip的mask, 未经过softmax,这个是来自backbone的mask,存在上限,引导
         聚类：
            1. 根据paca_vit:做cluster_center ,这个引导作用应该在每个block块切块之前,不扩展到mask   ------ cluster_guide √
                (cgrseg)
                直接对原始数据进行引导, (classes,N)@(d,N).transpose(-1,-2) -> (classes,d)
                (classes,d).transpose(-1,-2) @ channelAtten((classes,d)) -> (d,1) -> mlp(d,1) -> (d,1) -> sigmoid -> (d,1)
                (d,n)*(d,1) -> (d,1)
                
                减小计算量                                                                     ----- cluster_reduce_cmp √
                    (classes,N)@(d,N).transpose(-1,-2) -> (classes,d) #A->B对齐
                    Cross_atten(T(classes,d),T-1(classes,d)) -> (classes,d)
                    (classes,d).transpose(-1,-2) @ (classes,N) = (d,N)  #B->A对齐   
                
            2. Agent Cluster Center                                                         ---------cluster_Agent √
                softmax(Tq @ Center_t) @ (softmax(T-1_k @ Center_t-1_k) @ Center_t-1_v) 
                2.1 (classes,N) @ (N,d) -> (classes,d)
                2.2 (d,N) @ (N,classes) -> (d,classes)
                2.3 (N,d) @ (d,classes) -> (N,classes)
                2.4 (N,classes) @ (classes,d) -> (N,d)
        
        掩码(用在ipsa中):但是问题有argmax不能更新,需要处理                                    --------- mask_guide
            1. Atten(i,:)+负无穷; Atten(:,j)+负无穷; when cls(i) = cls(j) = background
            2. patch内部,Atten(i,j) + 负无穷 ; when cls(i) != cls(j)

        top-k掩码-相似度掩码(cpsa中,mrcfa)                                                ----------topk_mask
        ...
        if self.cross_method == 'CAT':
            ...
            for _ in range(self.num_layers):
                # print("in_model",output.shape,memory.shape,seg_mask.shape)
                output = self.pre_isa_blocks[0][_](output,memory,seg_mask)
        ...
        elif self.cross_method == 'Focal':
            _c_further = torch.cat([memory,src],dim=1) # [b,all_num_clips,c,h,w]
            output = self.decoder_focal(_c_further) #这里前几帧都没更新，只有最后一帧更新
            # print("focal out:",output.shape)
            num_clips+=1
