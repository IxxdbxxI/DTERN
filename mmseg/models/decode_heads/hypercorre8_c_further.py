...
class Cluster_layer(nn.Module):
    def __init__(self, dim, num_heads,  num_clusters,mlp_ratio=4., qkv_bias=True, 
                 qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,cluster_with_t=False,t=3):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.num_clusters = num_clusters
...
class hypercorre_topk2(nn.Module):
    ...
    def __init__(self,dim=[64, 128, 320, 512], num_layers=1, t=3, time_decoder_layer=3,embedding_dim=256,num_classes = 124,
                 cross_method='CAT',ratio_fusio = False,num_clusters=150,cluster_with_t=False):
        super().__init__()
        self.dim=dim
        ...
        if ratio_fusio and ratio_fusio != "sub":
        ...
        #             act_cfg=dict(type='ReLU'))
        #         )
            # if self.ratio_fusio == "sub":
            #     self.sub_convs.append(
            #             ConvModule(
            #                 in_channels=dim[idx],
            #                 out_channels=dim[idx],
            #                 kernel_size=3,
            #                 stride=1,
            #                 padding=1,
            #                 norm_cfg=dict(type='SyncBN', requires_grad=True),
        ...
        for i in range(num_layers):
            self.cluster_blocks.append(
                Cluster_layer(
                    ...
                    drop_path=0.,
                    act_layer=nn.GELU,
                    norm_layer=nn.LayerNorm,t=3,cluster_with_t=cluster_with_t)
            )
    ...
    def forward(self, query_frame, supp_frame,t = None,img_metas=None):
        ...
        out_supp_frame = []
        out_memory_frames = []
        T_pre = query_frame.shape[1]
        T_tg = supp_frame.shape[1]
        ...
        # out_supp_frame = torch.cat(out_supp_frame,dim=1) #[BT,C,H,W] 
        # out_memory_frames = torch.cat(out_memory_frames,dim=1) #[BT,C,H,W]
        src = supp_frame #[BT,C,H,W] 
        memory = query_frame #[BT,C,H,W]
