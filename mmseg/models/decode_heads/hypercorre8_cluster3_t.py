...
class Clustering(nn.Module):
    ...
    def forward(self, x,t,img_metas=None,centers_pre = None): #[bt,c,w,h] #t第一层为4，后面为1 , centers_pre为上一层的聚类中心#(bt*head*window,c,cw,ch)
        ...
        centers = centers.permute(0,3,1,5,6,2,4).reshape(-1,t*c_w*c_h,self.heads,c).type(torch.half) # (b*window,t*cw*ch,head,c)

        value = value.reshape(-1,t,self.heads,self.window_w*self.window_h, c, w, h) 
        value = value.permute(0,3,1,5,6,2,4).reshape(-1,t*w*h,self.heads,c).type(torch.half) # (b*window ,t*n,head,c)
        
        feature = feature.reshape(-1,t,self.heads,self.window_w*self.window_h, c, w, h) 
        feature = feature.permute(0,3,1,5,6,2,4).reshape(-1,t*w*h,self.heads,c).type(torch.half) # (b*window,t*n,head,c )
        ...
        # centers = centers.permute(0,2,3,1,4).contiguous().view(b//self.heads,t*c_w*c_h,self.heads,c).type(torch.half) #(b*window,t*n_avg,head,c) 
        # value = value.reshape(-1,self.heads, self.window_w*self.window_h, t*w*h,c) # (b,head,window,t,c)
        # value = value.permute(0,2,3,1,4).contiguous().view(b//self.heads,t*w*h,self.heads,c).type(torch.half) #(b*window,tn,head,c)
        # feature = feature.reshape(-1,self.heads, self.window_w*self.window_h, t*w*h,c) # (b,head,window,t,c)
        # feature = feature.permute(0,2,3,1,4).contiguous().view(b//self.heads,t*w*h,self.heads,c).type(torch.half) #(b*window,tn,head,c)
        ...
        # 加入交互增强互补性,这里为什么是和value和feature交互，考虑更换成SA，将时间视作多视图进行交互增强互补性,这里会把余弦相似度拉近，类间距非常小
        for _ in range(self.num_clustering):    # iterative clustering and updating centers
            centers = flash_attn_func(centers, value, feature) # all_t (b*window,t*n,head,c) # 这里可以测试近高亲合并
        # 前面feteature参与了centers的产生过程，不能去掉mem

        # 测试类距离
        if not self.training:
            centers_test = centers.reshape(b//self.heads//t,t*c_w*c_h,-1)
            print("before_centers",centers_test.shape)
            sim_centers =  pairwise_cos_sim(
                    centers_test.type(torch.float),
                    centers_test.type(torch.float)
                ) # [B,N,N]
            
            # 近亲均值合并
            centers_test = merge_by_similarity(centers_test, sim_centers,threshold=0.85)  #去除冗余 [B, M, c] 这里运行速度太慢了
            print("after_centers",centers_test.shape)
            
        frame_feature = feature.reshape(-1, self.window_w*self.window_h,t, w*h, self.heads, c) # (b*window,t*n,head,c)
        # print("frame_feature",frame_feature.shape)
        tg_feature = frame_feature[:,:,-1] # (b,window,w*h,head,c)
        tg_feature = tg_feature.permute(0,3,1,2,4).reshape(-1, w*h, c).type(torch.float) # (b*head*window,n,c)
        # print("tg_feature",tg_feature.shape)

        # processing after flash attention
        # tg_frame
        b_tg,_,_ = tg_feature.shape
        # 这里可以加上centers_pre进行avg_update
        centers_out = centers # (b*window,t*n,head,c)
        ...
        centers = centers.reshape(-1,self.window_w*self.window_h, t*c_w*c_h,self.heads,c)
        centers = centers.permute(0,3,1,2,4).reshape(b_tg,t*c_w*c_h,c) # (b*head*window,t*cw*ch,c)
        ...
        # 为什么是center当作Q
        M, N = centers_feature.shape[1], tg_feature.shape[1]
        ...
        out = (out + centers_feature) / (mask.sum(dim=-1, keepdim=True) + 1.0)
