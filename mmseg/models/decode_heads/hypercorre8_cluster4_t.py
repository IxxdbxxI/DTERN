...
def merge_by_similarity(tensor, similarity_matrix, threshold=0.9):
    B, N, C = tensor.shape

    merged_results = []
    
    for b in range(B):
        sim_mat = similarity_matrix[b]  # 当前 batch 的相似度矩阵 (N, N)
        features = tensor[b]  # 当前 batch 的特征向量 (N, C)
        
        # 相似度矩阵二值化，生成连通分量的邻接关系
        adjacency = (sim_mat > threshold).float()
        
        # 使用矩阵幂来计算连通分量（基于 Warshall 算法的思想）
        with torch.no_grad():
            # 连通性传播，找到所有直接或间接相连的节点
            while True:
                prev_adjacency = adjacency.clone()
                adjacency = torch.matmul(adjacency, adjacency)
                adjacency = (adjacency > 0).float()  # 取 0-1 矩阵
                if torch.equal(prev_adjacency, adjacency):
                    break
        
        # 连通分量标签分组（每个节点对应一个分量标签）
        group_ids = torch.zeros(N, dtype=torch.long, device=tensor.device)
        visited = torch.zeros(N, dtype=torch.bool, device=tensor.device)
        group_label = 0
        
        for i in range(N):
            if not visited[i]:
                # 找到当前节点 i 所属的分量
                connected = adjacency[i] > 0
                group_ids[connected] = group_label
                visited[connected] = True
                group_label += 1
        
        # 按组计算均值
        unique_groups = group_ids.unique()
        merged_features = torch.stack([
            features[group_ids == group].mean(dim=0) for group in unique_groups
        ])
        
        merged_results.append(merged_features)
    
    # 统一结果形状并返回
    max_components = max([res.shape[0] for res in merged_results])
    final_tensor = torch.zeros(B, max_components, C, device=tensor.device)
    for b, res in enumerate(merged_results):
        final_tensor[b, :res.shape[0], :] = res
...
class Clustering(nn.Module):
    def __init__(self, dim, out_dim, center_w=2, center_h=2, window_w=2, window_h=2, heads=4, head_dim=24, return_center=False, num_clustering=1):
        ...
        self.linear2 = nn.Linear(head_dim, head_dim)
        self.sim_alpha = nn.Parameter(torch.ones(1))
        self.sim_beta = nn.Parameter(torch.zeros(1))
        self.centers_proposal = nn.AdaptiveAvgPool2d((center_w,center_h)) #不符合语义的方式，采用CNN更加符合语义的形式，分割这里应该是语义聚类
    ...
    def forward(self, x,t,img_metas=None,centers_pre = None): #[bt,c,w,h] #t第一层为4，后面为1 , centers_pre为上一层的聚类中心#(bt*head*window,c,cw,ch)
        _,_,W,H = x.shape
        ...
        centers_feature = centers_feature.permute(0,2,3,1,5,6,4).reshape(-1,t*c_w*c_h,c) # (b*head*window,t*n_avg,c)
        
        centers = centers.reshape(-1,t,self.heads,self.window_w*self.window_h, c, c_w, c_h)
        centers = centers.permute(0,2,3,1,5,6,4).reshape(-1,t*c_w*c_h,c) # (b*head*window,t*cw*ch,c)
        
        feature = feature.reshape(-1,t,self.heads,self.window_w*self.window_h, c, w, h) # (b*head*window,t,n,c)
        feature = feature.permute(0,2,3,1,5,6,4).reshape(-1,t*w*h,c) # (b*head*window,t*n,c )

        value_centers = self.linear1(centers) # (b*head*window,t*n,c)
        key_centers = self.linear2(centers) # (b*head*window,t*n,c)

        # print("centers",centers.shape)
        # processing before flash attention
        centers = centers.reshape(-1,self.heads, self.window_w*self.window_h, t*c_w*c_h,c) # (b,head,window,t,c)
        centers = centers.permute(0,2,3,1,4).contiguous().view(b//self.heads//t,t*c_w*c_h,self.heads,c).type(torch.half) #(b*window,t*n_avg,head,c) 

        value_centers = value_centers.reshape(-1,self.heads, self.window_w*self.window_h, t*c_w*c_h,c) # (b,head,window,t,c)
        value_centers = value_centers.permute(0,2,3,1,4).contiguous().view(b//self.heads//t,t*c_w*c_h,self.heads,c).type(torch.half) #(b*window,tn,head,c)
        key_centers = key_centers.reshape(-1,self.heads, self.window_w*self.window_h, t*c_w*c_h,c) # (b,head,window,t,c)
        key_centers = key_centers.permute(0,2,3,1,4).contiguous().view(b//self.heads//t,t*c_w*c_h,self.heads,c).type(torch.half) #(b*window,tn,head,c)
        ...
        # # 加入交互增强互补性SA 这里实验效果把类间距拉的很近，改成CA试一试
        # for _ in range(self.num_clustering):    # iterative clustering and updating centers 这里会拉近类间距
        #     centers = flash_attn_func(centers, key_centers, value_centers) # all_t (b*window,t*n,head,c) # 这里可以测试近高亲合并


        # 这里可以加上centers_pre进行avg_update,如果这里就返回了，center是均值出来的结果，监督sa的过程,这里由于存在差异，监督的程度需要弱

        # 样本级的合并
        centers = centers.reshape(b//self.heads//t,t*c_w*c_h,-1)
        centers_out = centers # # #(b*window,tn,C)

        # sim_centers =  pairwise_cos_sim(
        #         centers.type(torch.float),
        #         centers.type(torch.float)
        #     ) # [B,N,N]
        
        # # 近亲均值合并
        # centers = merge_by_similarity(centers, sim_centers,threshold=0.85)  #去除冗余 [B, M, c] 这里运行速度太慢了
        
        _,m,_ = centers.shape
        # print("centers",centers.shape) # (b*head*window,t*n,c )
        frame_feature = feature.reshape(-1, t, w*h,  c) # (b*head*window,t*n,c)
        # print("frame_feature",frame_feature.shape)
        tg_feature = frame_feature[:,-1] # (b*head*window,n,c)
        tg_feature = tg_feature.type(torch.float) # (b*head*window,n,c)
        # print("tg_feature",tg_feature.shape)

        # processing after flash attention
        # tg_frame
        b_tg,_,_ = tg_feature.shape
        #x [bt*head*window, c, w, h]
        frame_x = x.reshape(-1,t,self.heads,self.window_w*self.window_h, c, w, h)
        tg_x = frame_x[:,-1,:,:,:,:,:].reshape(-1, w*h, c) # (b*head*window,n,c) 
        # print("centers,value,tg_feature,tg_x",centers.shape,value.shape,tg_feature.shape,tg_x.shape)

        centers = centers.reshape(-1,self.window_w*self.window_h, m,self.heads,c)
        centers = centers.permute(0,3,1,2,4).reshape(b_tg,m,c) # (b*head*window,t*cw*ch,c)
        ...
        # print("centers,tg_x",centers.shape,tg_x.shape)
        similarity = torch.sigmoid(self.sim_beta + self.sim_alpha * pairwise_cos_sim(centers.type(torch.float), tg_x.type(torch.float)))
     
     
        # # --------------------------------------------------------method from efc----------------------------------------------------------
        # _, sim_max_idx = similarity.max(dim=1, keepdim=True) 
        # mask = torch.zeros_like(similarity)  # binary #[B,M,N]
        # mask.scatter_(1, sim_max_idx, 1.)
        # similarity = similarity * mask
        # # tg_feature:(b*head*window,n,c) centers_feature:# (b*head*window,t*n_avg,c)
        # # value2 = rearrange(value, 'b c w h -> b (w h) c')  # [B,N,D]
        # # aggregate step, out shape [B,M,D]
        # M, N = centers.shape[1], tg_feature.shape[1]
        # tg_feature = rearrange(tg_feature, 'b n c -> (b n) c')
        # sim_max_idx = rearrange(sim_max_idx.squeeze(1), 'b n -> (b n)') # [B,N]
        # # b:b*head*window  M:t*n_avg  N:n
        # idx_offset = (torch.arange(b_tg, device=sim_max_idx.device) * M).unsqueeze(-1).expand(-1, N).flatten() #[b*head*window,t*n_avg,n] -> [b*head*window*t*n_avg*n]
        # sim_max_idx = sim_max_idx + idx_offset
        
        # # out: [b*head*window, t*n_avg, c]
        # out = rearrange(scatter_sum(tg_feature, sim_max_idx, dim=0, dim_size=b_tg*M), '(b m) c -> b m c', b=b_tg, m=M)  # Different from CoC's implementation "(value2.unsqueeze(dim=1) * sim.unsqueeze(dim=-1)).sum(dim=2)", we use scatter_sum to avoid OOM.
        # out = (out + centers) / (mask.sum(dim=-1, keepdim=True) + 1.0)


        ##---------------------------------------------------------methods from clusterformer-----------------------------------------------------
        ...
        out = ((tg_feature.unsqueeze(dim=1)*similarity.unsqueeze(dim=-1) ).sum(dim=2) + centers_feature)/ (mask.sum(dim=-1,keepdim=True)+ 1.0) 


        if self.return_center:
            out = out.reshape(b_tg, c, c_w, c_h)
            return out
        else:
        ...
        out = self.proj(out) # (b,c,w,h)
        return out,centers_out
...
class ClusterBlock(nn.Module):
    ...
    def forward(self, x,H,W,mem=None,t=1,img_metas=None): #[bt,n,c]
        ...
        if self.use_layer_scale:
            x1,centers = self.token_mixer(self.norm1(x),t,img_metas=img_metas)
            out1 = self.drop_path(
            ...
            x1 = self.token_mixer(self.norm1(x),t,img_metas=img_metas)
            x = tg_x + self.drop_path(x1)
...
class hypercorre_topk2(nn.Module):
    ...
    def __init__(self,dim=[64, 128, 320, 512], num_layers=1, t=3, time_decoder_layer=3,embedding_dim=256,num_classes = 124,cross_method='CAT',ratio_fusio = False,num_clusters=150):
        ...
        num_heads = [2,4,8,16]
        self.cluster_heads = 8
        # 后面测试一下直接用fc试一试，这里大小很难测试
        self.window_h = 2
        self.window_w = 2
        self.cluster_h = 12
        self.cluster_w = 12 
        ...
        self.cluster_blocks = basic_blocks(
                                    ...
                                    center_h=self.cluster_h,
                                    center_w=self.cluster_w,
                                    window_h=self.window_h,
                                    window_w=self.window_w,
                                    heads=self.cluster_heads,
                                    head_dim=self.embedding_dim//self.cluster_heads,
                                )
    ...
    def forward(self, query_frame, supp_frame,img_metas = None,t=None):
        ...
        for idx,blk in enumerate(self.cluster_blocks):
            if idx == 0:
                x,centers = blk(src, H=H, W=W, mem = memory, t=T_pre,img_metas=img_metas)
            elif idx==1:
                x,centers = blk(src, H=H, W=W, mem = memory, t=T_pre,img_metas=img_metas)
            else:
        ...
        # ## # #(b*window,tn,head,c)
        centers = centers.reshape(B,self.window_w*self.window_h,T_pre+T_tg,self.cluster_w*self.cluster_h,self.cluster_heads,self.embedding_dim//self.cluster_heads)
        centers = centers.permute(0,2,1,3,4,5).reshape(B,T_pre+T_tg,-1,C) # (B,T_pre+T_tg,H*W,C)
