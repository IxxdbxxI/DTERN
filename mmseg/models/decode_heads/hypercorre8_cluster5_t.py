...
class Channel_Attention(nn.Module):
    ...
    def forward(self, x,t): #[b,tn,c]
        ...
        _max,_ = x.max(dim=-1,keep_dim=True)
        _avg = x.mean(dim=-1,keep_dim=True)
        glb = _max + _avg #[b,t]
        glb = self.mlp(glb)
...
class Cluster_layer(nn.Module):
    def __init__(self, dim, num_heads,  num_clusters,mlp_ratio=4., qkv_bias=True, 
        ...
        if self.cluster_with_t:
            print("using cluster_with_t by channel attention, t:{}".format(t)) 
            self.fusion = Channel_Attention(t)
    ...
    def forward(self,x,H,W,z=None,mem=None,t=3):
        ...
        # 交互t?直接求平均捏，或者求一个加权和 TDSNet 0.038 0.075
        C_in = rearrange(cluster_x_z,"(b t) n c -> b (t n) c",t=t,b=res.shape[0])

        if self.cluster_with_t:
            C_in = self.fusion(C_in,t)
...
# ---------------------------------------------------only target agreement clusterformer--------------------------------------------------
        '''
            2.这里只采用了target进行聚类中心的聚合,
        '''
        #  (b,num_clusters,c) (b,n,c) -> (b,num_clusters,n)
        similarity = torch.sigmoid(self.sim_beta + self.sim_alpha * pairwise_cos_sim(C_in.type(torch.float), res.type(torch.float))) # res(更符合特征聚合的思想) / feature
...
#-----------------------------------------------------只在乎分配,这里会产生多个点一样的特征的情况------------------------------------------------------------
        ...
        # # (b,n,n_cluster)(b,n_cluster,c) -> (b,n,c) -> (b,num_clusters,c)
        # out = similarity @ C_in  
# ---------------------------------------------------------------------------------------------------------------------------------------
...
class hypercorre_topk2(nn.Module):
    ...
    def __init__(self,dim=[64, 128, 320, 512], num_layers=1, t=3, time_decoder_layer=3,embedding_dim=256,
                 num_classes = 124,cross_method='CAT',ratio_fusio = False,num_clusters=150,cluster_with_t=False):
        super().__init__()
        self.dim=dim
    ...
    def forward(self, query_frame, supp_frame,t = None,img_metas=None):
        ...
        # z: cluster # [bt,n,num_clusters]
        z = rearrange(z, '(b t) n c -> b t c n', b=B, t=T_tg+T_pre) # 用于对比损失
        ...
        return out_new,out_cls_mid.squeeze(1),z #[b,c,h,w]
