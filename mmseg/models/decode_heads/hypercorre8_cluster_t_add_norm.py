...
class Cluster_layer(nn.Module):
    def __init__(self, dim, num_heads,  num_clusters,mlp_ratio=4., qkv_bias=True, 
        ...
        # self.attn = nn.MultiheadAttention(dim, num_heads, attn_drop=0.)
    ...
    def forward(self,x,H,W,z=None,mem=None,t=3):
        ...
        src_res = self.norm1(res)
        src = rearrange(src_res,"b n c -> n b c")
        mem = rearrange(C_in, "b n c -> n b c")
        
        # out,_ = self.attn(src, mem, mem)
        ...
        out = res + out
        out = out + self.mlp(self.norm1(out),H,W)
